...
{
    SECTION("pawn") {
        board.import_fen( "3Q4/5B2/1N6/3p4/2P1K3/8/8/3R3R w - - 0 1");                              // white king
        generate_and_sort(board, s::e4, v);
        CHECK(v == std::vector<Square>{s::f3, s::e3, s::d3, s::f4, s::d4, s::f5, s::e5, s::d5});
        ...
        CHECK(v == std::vector<Square>{s::d5});
        board.import_fen( "3q4/8/1np1k3/3P4/8/5b2/8/3r3r w - - 0 1");                              // black king
        generate_and_sort(board, s::e6, v);
        CHECK(v == std::vector<Square>{s::f5, s::e5, s::d5, s::f6, s::d6, s::f7, s::e7, s::d7});
        generate_and_sort(board, s::h1, v);
        CHECK(v == std::vector<Square>{});
        ...
        board.import_fen( "3Q4/5B2/1N6/3r2K1/2P5/8/8/3R3R w - - 0 1");                              // white king
        ...
        generate_and_sort(board, s::d8, v);
        CHECK(v == std::vector<Square>{s::d5});
        board.import_fen( "3q4/5b2/1np5/3R2k1/8/8/8/3r3r w - - 0 1");                              // black king
        generate_and_sort(board, s::g5, v);
        CHECK(v == std::vector<Square>{s::h4, s::g4, s::f4, s::h6, s::g6, s::f6});
        generate_and_sort(board, s::h1, v);
        CHECK(v == std::vector<Square>{});
        ...
        board.import_fen( "3Q4/5B2/1N6/3b4/2P5/8/6K1/3R3R w - - 0 1");                              // white king
        ...
        generate_and_sort(board, s::d8, v);
        CHECK(v == std::vector<Square>{s::d5});
        board.import_fen( "3q4/5b2/1np5/3B4/8/8/6k1/3r3r w - - 0 1");                              // black king
        generate_and_sort(board, s::g2, v);
        CHECK(v == std::vector<Square>{s::g1, s::f1, s::h2, s::f2, s::h3, s::g3});
        generate_and_sort(board, s::h1, v);
        CHECK(v == std::vector<Square>{});
        ...
        board.import_fen( "3Q4/5B2/1N6/3n4/2P2K2/8/8/3R3R w - - 0 1");                              // white king
        ...
        generate_and_sort(board, s::d8, v);
        CHECK(v == std::vector<Square>{s::d5});
        board.import_fen( "3q4/5b2/1np5/3N4/5k2/8/8/3r3r w - - 0 1");                              // black king
        generate_and_sort(board, s::f4, v);
        CHECK(v == std::vector<Square>{s::g3, s::f3, s::g4, s::e4, s::g5, s::f5, s::e5});
        generate_and_sort(board, s::h1, v);
        CHECK(v == std::vector<Square>{});
...
TEST_CASE("blocking")
{
    SECTION("vertical attacks") {
        board.import_fen( "8/4r3/8/2R5/3Q1BN1/8/4K3/8 w - - 0 1");          // white king
        ...
        board.import_fen( "8/4k3/8/2r5/3q1bn1/8/4R3/8 w - - 0 1");          // black king
        ...
        board.import_fen( "8/8/3NB3/8/r6K/5P2/3QR1P1/8 w - - 0 1");        // white king
        ...
        board.import_fen( "8/3qr1p1/5p2/k6R/8/3nb3/8/8 w - - 0 1");        // black king
        ...
        board.import_fen( "8/5b2/2QR4/8/3P1N2/1K1B4/2P5/8 w - - 0 1");         // white king
        ...
        board.import_fen( "8/2p5/1k1b4/3p1n2/8/2qr4/5B2/8 w - - 0 1");         // black king
        generate_and_sort(board, s::b6, v);
...
{
    board.import_fen( "8/5b2/2QR4/1r6/3P1N2/1K1B4/2P5/1n6 w - - 0 1");         // white king
    generate_and_sort(board, s::b3, v);
    ...
    generate_and_sort(board, s::c6, v);
    CHECK(v == std::vector<Square>{});
    board.import_fen( "1N6/2p5/1k1b4/3p1n2/8/1Rqr4/5B2/8 w - - 0 1");             // black king
    generate_and_sort(board, s::b6, v);
