...
void generate_and_sort_mbc(Board& board, const Square& sq, std::vector<Square>& v)
{
    v.clear();
...
TEST_CASE("capture with king")
{
    SECTION("pawn") {
        ...
        CHECK(v_mbc == std::vector<Square>{s::f3, s::e3, s::d3, s::f4, s::d4, s::f5, s::e5, s::d5});
        board_mbc.import_fen("8/8/8/3p4/2K5/8/8/8 w - - 0 1");
        generate_and_sort_mbc(board_mbc, s::c4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d3, s::c3, s::b3, s::d4, s::b4, s::d5, s::c5, s::b5});
        board_mbc.import_fen("8/8/4k3/3P4/8/8/8/8 w - - 0 1");                                        // black king
        generate_and_sort_mbc(board_mbc, s::e6, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::f5, s::e5, s::d5, s::f6, s::d6, s::f7, s::e7, s::d7});
        board_mbc.import_fen("8/8/2k5/3P4/8/8/8/8 w - - 0 1");
        generate_and_sort_mbc(board_mbc, s::c6, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5, s::c5, s::b5, s::d6, s::b6, s::d7, s::c7, s::b7});
    }SECTION("rook") {
        ...
        generate_and_sort_mbc(board_mbc, s::c4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d3, s::c3, s::d4, s::b4, s::d5, s::c5, s::b5});
        board_mbc.import_fen("8/8/8/3b4/4K3/8/8/8 w - - 0 1");
        generate_and_sort_mbc(board_mbc, s::e4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::e3, s::d3, s::f4, s::d4, s::f5, s::e5, s::d5});
        board_mbc.import_fen("8/8/4K3/3b4/8/8/8/8 w - - 0 1");
        generate_and_sort_mbc(board_mbc, s::e6, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::f5, s::e5, s::d5, s::f6, s::d6, s::e7, s::d7});
        board_mbc.import_fen("8/8/2K5/3b4/8/8/8/8 w - - 0 1");
        generate_and_sort_mbc(board_mbc, s::c6, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5, s::c5, s::b5, s::d6, s::b6, s::d7, s::c7});
...
{
    SECTION("pawn") {
        board_mbc.import_fen("3Q4/5B2/1N6/3p4/2P1K3/8/8/3R3R w - - 0 1");                              // white king
        generate_and_sort_mbc(board_mbc, s::e4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::f3, s::e3, s::d3, s::f4, s::d4, s::f5, s::e5, s::d5});
        ...
        CHECK(v_mbc == std::vector<Square>{s::d5});
        board_mbc.import_fen("3q4/8/1np1k3/3P4/8/5b2/8/3r3r w - - 0 1");                              // black king
        generate_and_sort_mbc(board_mbc, s::e6, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::f5, s::e5, s::d5, s::f6, s::d6, s::f7, s::e7, s::d7});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        ...
        board_mbc.import_fen("3Q4/5B2/1N6/3r2K1/2P5/8/8/3R3R w - - 0 1");                              // white king
        generate_and_sort_mbc(board_mbc, s::g5, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::h4, s::g4, s::f4, s::h6, s::g6, s::f6});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        generate_and_sort_mbc(board_mbc, s::d1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        ...
        generate_and_sort_mbc(board_mbc, s::d8, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        board_mbc.import_fen("3q4/5b2/1np5/3R2k1/8/8/8/3r3r w - - 0 1");                              // black king
        generate_and_sort_mbc(board_mbc, s::g5, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::h4, s::g4, s::f4, s::h6, s::g6, s::f6});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        ...
        board_mbc.import_fen("3Q4/5B2/1N6/3b4/2P5/8/6K1/3R3R w - - 0 1");                              // white king
        generate_and_sort_mbc(board_mbc, s::g2, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::g1, s::f1, s::h2, s::f2, s::h3, s::g3});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        generate_and_sort_mbc(board_mbc, s::d1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        ...
        generate_and_sort_mbc(board_mbc, s::d8, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        board_mbc.import_fen("3q4/5b2/1np5/3B4/8/8/6k1/3r3r w - - 0 1");                              // black king
        generate_and_sort_mbc(board_mbc, s::g2, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::g1, s::f1, s::h2, s::f2, s::h3, s::g3});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        ...
        board_mbc.import_fen("3Q4/5B2/1N6/3n4/2P2K2/8/8/3R3R w - - 0 1");                              // white king
        generate_and_sort_mbc(board_mbc, s::f4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::g3, s::f3, s::g4, s::e4, s::g5, s::f5, s::e5});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
        generate_and_sort_mbc(board_mbc, s::d1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        ...
        generate_and_sort_mbc(board_mbc, s::d8, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::d5});
        board_mbc.import_fen("3q4/5b2/1np5/3N4/5k2/8/8/3r3r w - - 0 1");                              // black king
        generate_and_sort_mbc(board_mbc, s::f4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::g3, s::f3, s::g4, s::e4, s::g5, s::f5, s::e5});
        generate_and_sort_mbc(board_mbc, s::h1, v_mbc);
        CHECK(v_mbc == std::vector<Square>{});
...
TEST_CASE("blocking")
{
    SECTION("vertical attacks") {
        ...
        CHECK(v_mbc == std::vector<Square>{s::f1, s::d1, s::f2, s::d2, s::f3, s::d3});
        ...
        CHECK(v_mbc == std::vector<Square>{s::f6, s::d6, s::f7, s::d7, s::f8, s::d8});
        generate_and_sort_mbc(board_mbc, s::g4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::e3, s::e5});
        generate_and_sort_mbc(board_mbc, s::f4, v_mbc);
        CHECK(v_mbc == std::vector<Square>{s::e3, s::e5});
        ...
        board_mbc.import_fen("8/5b2/2QR4/8/3P1N2/1K1B4/2P5/8 w - - 0 1");         // white king
        ...
        board_mbc.import_fen("8/2p5/1k1b4/3p1n2/8/2qr4/5B2/8 w - - 0 1");         // black king
        generate_and_sort_mbc(board_mbc, s::b6, v_mbc);
...
{
    board_mbc.import_fen("8/5b2/2QR4/1r6/3P1N2/1K1B4/2P5/1n6 w - - 0 1");         // white king
    generate_and_sort_mbc(board_mbc, s::b3, v_mbc);
    ...
    generate_and_sort_mbc(board_mbc, s::c6, v_mbc);
    CHECK(v_mbc == std::vector<Square>{});
    board_mbc.import_fen("1N6/2p5/1k1b4/3p1n2/8/1Rqr4/5B2/8 w - - 0 1");             // black king
    generate_and_sort_mbc(board_mbc, s::b6, v_mbc);
