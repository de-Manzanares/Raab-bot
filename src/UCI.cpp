...
/**
 * @brief Implements the main loop that listens to UCI commands and processes
 * them.
 */
void uci::loop() {
  ...
  while (std::getline(std::cin, in)) {
    ...
    if (simon_says(&in, "position")) {
      ...
      std::vector<Node *> opt_nodes{
          Search::min_max(n, D, neg_inf, pos_inf, maxing)};
      ...
      std::cout << "info"
                ...
                << " nodes " << Counter::node << "\nbestmove "
                << long_algebraic_notation(moves[0]) << std::endl;
...
/**
 * @brief Decomposes long algebraic notation into a format this program can
 * process.
...
/**
 * @brief Performs moves on a board as dictated by a string of long algebraic
 * notation
