 ...
 * @details This function updates the bitboards for each piece type by clearing
 * the bit corresponding to the given square. The piece_map is then updated to
 * reflect the new bitmaps.
 ...
 * @details This function places the specified piece on the given square of the
 * chessboard. The piece is identified by a character, and the square is
...
/**
 * @brief Check if a square contains a piece of the same color as the specified
 * color
 * @param sq The square to check
 ...
 */
bool Board::is_inSameRow(Square sq1, Square sq2) {
 ...
 */
bool Board::is_inSameColumn(Square sq1, Square sq2) {
 ...
 * @details A set bit means the a piece is on the board in that square, 0 bit
 * means 0 piece
...
/**
 * @brief Computes the influence of a rook on the board, given a specific
 * square.
 * @param sq The square to compute the influence for.
 * @return std::vector<Square> A vector of squares representing the influence of
 * the rook.
 */
 ...
 * @return std::vector<Square> A vector containing all the squares influenced by
 * the bishop.
 */
...
std::vector<Square> Board::influence_knight(Square sq) {
  ...
  if (isq >= 18 && isq <= 21 || isq >= 26 && isq <= 29 ||
  ...
  // b file - 4 squares
  else if (sq == Square::b3 || sq == Square::b4 || sq == Square::b5 ||
           sq == Square::b6) {
    influence.push_back(sq - (2 * 8 + 1)); // down right
    influence.push_back(sq - (2 * 8 - 1)); // down left
    influence.push_back(sq - (8 + 2));     // right down
  ...
  // g files - 4 squares
  else if (sq == Square::g3 || sq == Square::g4 || sq == Square::g5 ||
           sq == Square::g6) {
    influence.push_back(sq - (2 * 8 + 1)); // down right
    influence.push_back(sq - (2 * 8 - 1)); // down left
    influence.push_back(sq - (8 - 2));     // left down
  ...
  // 2nd rank - 4 squares
  else if (isq >= static_cast<int>(Square::f2) &&
           isq <= static_cast<int>(Square::c2)) {
    influence.push_back(sq - (8 + 2));     // right down
    influence.push_back(sq - (8 - 2));     // left down
    influence.push_back(sq + (8 - 2));     // right up
    influence.push_back(sq + (8 + 2));     // left up
  ...
  // 7th rank - 4 squares
  else if (isq >= static_cast<int>(Square::f7) &&
           isq <= static_cast<int>(Square::c7)) {
    influence.push_back(sq - (2 * 8 + 1)); // down right
    influence.push_back(sq - (2 * 8 - 1)); // down left
    influence.push_back(sq - (8 + 2));     // right down
    influence.push_back(sq - (8 - 2));     // left down
  ...
  // a file - 4 squares
  else if (sq == Square::a3 || sq == Square::a4 || sq == Square::a5 ||
           sq == Square::a6) {
    influence.push_back(sq - (2 * 8 + 1)); // down right
    influence.push_back(sq - (8 + 2));     // right down
    influence.push_back(sq + (8 - 2));     // right up
  ...
  // h file - 4 squares
  else if (sq == Square::h3 || sq == Square::h4 || sq == Square::h5 ||
           sq == Square::h6) {
    influence.push_back(sq - (2 * 8 - 1)); // down left
    influence.push_back(sq - (8 - 2));     // left down
    influence.push_back(sq + (8 + 2));     // left up
  ...
  // 1st rank - 4 squares
  else if (isq >= static_cast<int>(Square::f1) &&
           isq <= static_cast<int>(Square::c1)) {
    influence.push_back(sq + (8 - 2));     // right up
    influence.push_back(sq + (8 + 2));     // left up
    influence.push_back(sq + (2 * 8 - 1)); // up right
    influence.push_back(sq + (2 * 8 + 1)); // up left
  ...
  // 8th rank - 4 squares
  else if (isq >= static_cast<int>(Square::f8) &&
           isq <= static_cast<int>(Square::c8)) {
    influence.push_back(sq - (2 * 8 + 1)); // down right
    influence.push_back(sq - (2 * 8 - 1)); // down left
    influence.push_back(sq - (8 + 2));     // right down
    influence.push_back(sq - (8 - 2));     // left down
  ...
  else if (isq == static_cast<int>(Square::g2)) {
...
/**
 * @brief Calculates the squares that are influenced by the king at a given
 * square.
...
/**
 * @brief Creates a list of squares that a given piece is currently
 * "influencing".
 ...
 * This function resets the influence_map_white and influence_map_black maps,
 * and then calculates the influence of each white and black piece on the board.
 * The influence is the set of squares that a piece can attack or control.
 */
 ...
 * @details This function searches the diagonal directions outward from a given
 * bishop to see if the bishop is pinning any enemy pieces.
...
/**
 * @brief Detects if a piece at the given square is pinning an opposing piece to
 * the enemy king
 * @param sq The square to check
 ...
 * @details Checks every piece to find pins to the enemy king and records pinned
 * pieces
...
/**
 * @brief Applies rules of the games to reduce the list of all possible moves
 *for the white king to a list of legal moves for the white king
 * @param sq_w_King The current position of the white king.
 * @return A vector of squares containing pieces that are giving check to the
 *white king.
...
std::vector<Square> Board::update_white_king_moves(Square sq_w_King) {
  ...
  for (const auto &s : influence_kings) {
    ...
    for (const auto &pair : maps->black_influence) {
      if (std::find(pair.second.begin(), pair.second.end(), s) !=
          pair.second.end()) {
        under_attack = true;
        break;
      }
  ...
  if (game_state.castle_w_K) {
    if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(), s::g1) !=
        legal_moves_kings.end()) {
      if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(),
                    s::f1) == legal_moves_kings.end()) {
        // remove g1
        auto it = std::remove(legal_moves_kings.begin(),
                              legal_moves_kings.end(), s::g1);
        legal_moves_kings.erase(it, legal_moves_kings.end());
      }
    }
  }
  if (game_state.castle_w_Q) {
    if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(), s::c1) !=
        legal_moves_kings.end()) {
      if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(),
                    s::d1) == legal_moves_kings.end()) {
        // remove c1
        auto it = std::remove(legal_moves_kings.begin(),
                              legal_moves_kings.end(), s::c1);
        legal_moves_kings.erase(it, legal_moves_kings.end());
      }
...
/**
 * @brief Applies rules of the games to reduce the list of all possible moves
 *for the black king to a list of legal moves for the black king
...
std::vector<Square> Board::update_black_king_moves(Square sq_b_king) {
  ...
  // necessary to legally castle remove the castling square
  if (game_state.castle_b_k) {
    if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(), s::g8) !=
        legal_moves_kings.end()) {
      if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(),
                    s::f8) == legal_moves_kings.end()) {
        // remove g8
        auto it = std::remove(legal_moves_kings.begin(),
                              legal_moves_kings.end(), s::g8);
        legal_moves_kings.erase(it, legal_moves_kings.end());
      }
    }
  }
  if (game_state.castle_b_q) {
    if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(), s::c8) !=
        legal_moves_kings.end()) {
      if (std::find(legal_moves_kings.begin(), legal_moves_kings.end(),
                    s::d8) == legal_moves_kings.end()) {
        // remove c1
        auto it = std::remove(legal_moves_kings.begin(),
                              legal_moves_kings.end(), s::c8);
        legal_moves_kings.erase(it, legal_moves_kings.end());
      }
 ...
 * square of (a) king because we will need that info later, and now is as good a
 * time as any to get it.
 * @param c Indicates which map to copy
 * @param from A pointer to the unordered_map from which the squares will be
 * copied
...
Square Board::copy_influence(
    const Color c, std::unordered_map<Square, std::vector<Square>> *from,
    std::unordered_map<Square, std::vector<Square>> *to) const {
  Square sq_king{};
  to->clear();
  if (c == c::white) {
    for (const auto &pair : *from) {
      if (!is_white_pawn(pair.first) && !is_white_king(pair.first)) {
        to->insert(pair);
      } else if (is_white_king(pair.first)) {
        sq_king = pair.first;
      }
    }
  } else if (c == c::black) {
    for (const auto &pair : *from) {
      if (!is_black_pawn(pair.first) && !is_black_king(pair.first)) {
        to->insert(pair);
      } else if (is_black_king(pair.first)) {
        sq_king = pair.first;
      }
    }
  }
 ...
 * @param map A pointer to the unordered_map storing squares and their
 * corresponding vectors of squares
...
/**
 * @brief Move a pawn.
 * @details Check for promotion, two square move, or en passant, then move the
 * piece.
...
void Board::move_pawn(Square from, Square to, char ch) {
  ...
  if (ch == 'q' || ch == 'r' || ch == 'b' || ch == 'n') {
    ...
    if (promoted) {
      remove_piece(to);
      place_piece(to, c == Color::white ? static_cast<char>(std::toupper(ch))
                                        : static_cast<char>(ch));
      remove_piece(from);
    }
  }
...
void Board::move_king(Square from, Square to) {
  ...
  if (is_white_king(from) && from == Square::e1) {
    if (game_state.castle_w_K && to == Square::g1 &&
        is_white_rook(Square::h1)) {
      remove_piece(to);
      place_piece(to, what_piece(from));
      remove_piece(from);
    ...
    } else if (game_state.castle_w_Q && to == Square::c1 &&
               is_white_rook(Square::a1)) {
      remove_piece(to);
      place_piece(to, what_piece(from));
      remove_piece(from);
  ...
  if (is_black_king(from) && from == Square::e8) {
    if (game_state.castle_b_k && to == Square::g8 &&
        is_black_rook(Square::h8)) {
      remove_piece(to);
      place_piece(to, what_piece(from));
      remove_piece(from);
    ...
    } else if (game_state.castle_b_q && to == Square::c8 &&
               is_black_rook(Square::a8)) {
      remove_piece(to);
      place_piece(to, what_piece(from));
      remove_piece(from);
...
void Board::move_rook(Square from, Square to) {
  // update castling rights
  if (game_state.castle_b_q && from == Square::a8 && is_black_rook(from)) {
    game_state.castle_b_q = false;
  } else if (game_state.castle_b_k && from == Square::h8 &&
             is_black_rook(from)) {
    game_state.castle_b_k = false;
  } else if (game_state.castle_w_Q && from == Square::a1 &&
             is_white_rook(from)) {
    game_state.castle_w_Q = false;
  } else if (game_state.castle_w_K && from == Square::h1 &&
             is_white_rook(from)) {
...
void Board::move(Square from, Square to, char ch) {
  ...
  if (is_pawn(from) || ((what_color(to) != Color::none &&
                         !is_same_color(from, what_color(to))))) {
