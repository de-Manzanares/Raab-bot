...
double Eval::castle_bonus(const Node *n)
{
    ...
    if ((n->_from == s::e1 && n->_to == s::g1 && n->_board->is_white_rook(s::f1))
            || (n->_from == s::e8 && n->_to == s::g8 && n->_board->is_black_rook(s::f8))) { score += CASTLE_BONUS; }

    else if ((n->_from == s::e1 && n->_to == s::c1 && n->_board->is_black_rook(s::d1))
            || (n->_from == s::e8 && n->_to == s::c8 && n->_board->is_black_rook(s::d8))) { score -= CASTLE_BONUS; }
...
double Eval::simple_evaluation(const Node *n)
{
    if (detect_checkmate(n) == 0) {
        double sum = material_evaluation(n)
                + mobility_evaluation(n)
                + check_bonus(n)
                + castle_bonus(n);
        return sum;
    }
